...
Validates unauthenticated discovery, OpenAPI specifications, documentation, and system
metrics endpoints.
...
- **Check 1.6**: `GET /v1/metrics` returns `200 OK` serving OpenMetrics formatted Prometheus gauges.
...
- **Check 2.1 (Invalid Key Handshake Rejection)**: Connects to `/websocket/{appName}/{invalidKey}`.
  Asserts that the upgrade is rejected before completion with HTTP 401 Unauthorized and an
  RFC 9457 Problem Details body (`application/problem+json`) containing `title` and `status`.

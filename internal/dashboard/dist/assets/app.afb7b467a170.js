(() => {
          ...
          <div class="card-body">
            ...
            <div style="margin-top:16px; padding:12px; background:var(--bg-tertiary); border:1px solid var(--border-color); border-radius:var(--radius-sm); font-size:12px; color:var(--text-secondary); display:flex; align-items:flex-start; gap:10px;">
              ...
              <div>
                ...
                <div>
                  ${activeExecutors > 0 ? `Operational telemetry and workflow dispatches are serviced via ${activeExecutors} connected ${activeExecutors === 1 ? "executor" : "executors"} connecting to the application system database. Relay coordinates executions without storing database credentials.` : `No executors are currently connected. Live dispatch and queue operations require an active executor connected to the application database (or direct data plane fallback configured in Relay server).`}
                </div>
              </div>
            </div>
          </div>
      ...
      try {
        ...
        <nav class="wf-breadcrumbs" aria-label="Workflow Navigation Breadcrumbs">
          ...
          <span class="wf-breadcrumb-current" aria-current="page">
            ...
            <span class="wf-breadcrumb-name">${escapeHtml4(workflowName)}</span>
          </span>
        </nav>
      `;
        el.innerHTML = `
        ...
        <div class="card">
          <div class="card-header">
            <div style="display:flex; align-items:center; gap:12px; min-width:0; overflow:hidden;">
              <span class="card-title" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:0;">Workflow: <code>${escapeHtml4(workflowId)}</code></span>
              ${renderStatusPill(wf.status)}
            </div>
            <div style="display:flex; gap:8px; flex-shrink:0;">
              ${wf.status === "PENDING" || wf.status === "ENQUEUED" ? `
                <button class="btn btn-sm btn-danger" data-cancel-wf='${escapeHtml4(workflowId)}' data-target-app='${escapeHtml4(targetApp)}'>Cancel</button>
              ` : ""}
              ${wf.status === "CANCELLED" ? `
                <button class="btn btn-sm btn-primary" data-resume-wf='${escapeHtml4(workflowId)}' data-target-app='${escapeHtml4(targetApp)}'>Resume</button>
              ` : ""}
              ${wf.status === "ERROR" ? `
                <button class="btn btn-sm btn-primary" data-restart-wf='${escapeHtml4(workflowId)}' data-target-app='${escapeHtml4(targetApp)}'>Restart</button>
              ` : ""}
            </div>
          </div>
          <div class="card-body">
            <div class="stat-grid" style="grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); margin-bottom: 0;">
              ...
              <div>
                ...
                <div style="word-break:break-all;"><strong>${escapeHtml4(workflowName)}</strong></div>
              </div>
              <div>
                <span class="stat-label">Queue</span>
                <div>${escapeHtml4(wf.queueName || wf.queue_name || "default")}</div>
              </div>
              <div>
    ...
    async renderMetricsScreen(el, silent = false) {
      ...
      try {
        ...
        for (const fam of families) {
          ...
          familyOptions.push(`<option value="${escapeHtml4(fam.name)}"${f.family === fam.name ? " selected" : ""}>${escapeHtml4(short)}</option>`);
        }
        let sections = "";
        ...
        <div class="toolbar">
          <div class="filter-group">
            <input type="text" id="metrics-application" class="input-text" placeholder="Application" aria-label="Filter by application" value="${escapeHtml4(f.application)}" style="width:180px;">
            <input type="text" id="metrics-workflow" class="input-text" placeholder="Workflow name" aria-label="Filter by workflow name" value="${escapeHtml4(f.workflowName)}" style="width:200px;">
        ...
        <div class="table-container">
          <table class="data-table">
            ...
            <tbody>
              ${rows.map((r) => `
                <tr>
                  <td><code>${escapeHtml4(r.workflow)}</code></td>
                  <td>${escapeHtml4(r.application)}</td>
        ...
        <div class="table-container">
          <table class="data-table">
            ...
            <tbody>
              ${rows.map((r) => `
                <tr>
                  <td><code>${escapeHtml4(r.queue || "n/a")}</code></td>
                  <td>${escapeHtml4(r.workflow)}</td>
        ...
        <div class="table-container">
          <table class="data-table">
            ...
            <tbody>
              ...
              ${stepRows.map(([step, v]) => `
                <tr>
                  <td><code>${escapeHtml4(step)}</code> <span class="text-secondary" style="font-size:11px;">(step)</span></td>
                  <td>-</td>
                  <td>-</td>
          ...
          <table class="data-table">
            ...
            <tbody>
              ${rows.map((r) => `
                <tr>
                  <td>${escapeHtml4(r.application)}</td>
                  <td><code>${escapeHtml4(r.version)}</code></td>
                  <td>${escapeHtml4(r.status)}</td>
        ...
        <div class="table-container">
          <table class="data-table">
            ...
            <tbody>
              ${rows.map((r) => `
                <tr>
                  <td><code>${escapeHtml4(r.step)}</code></td>
                  <td>${escapeHtml4(r.application)}</td>
                  <td>${formatMetricValue(r.success)}</td>
                  <td>${formatMetricValue(r.failed)}</td>
                  <td>${metricBar(r.success, max, "var(--color-success)")}</td>
  ...
  function metricBar(value, max, color) {
    ...
    return `<div class="metric-bar" role="img" aria-label="${escapeHtml4(label)}"><div class="metric-bar-fill" style="width:${pct.toFixed(1)}%;${color ? `background:${color};` : ""}"></div></div>`;
  }
